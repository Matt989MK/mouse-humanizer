...
class TypingEngine:
    ...
    def __init__(self):
        ...
        self.base_wpm = 85  # Words per minute (Gen Z speed)
    ...
    def _perform_copy_paste(self, text: str) -> bool:
        """Perform real copy-paste using clipboard and Ctrl+V/Cmd+V"""
        if not self.controller:
        ...
        try:
            pyperclip.copy(text)
            time.sleep(random.uniform(0.1, 0.3))  # Simulate paste delay

            # Determine the correct paste shortcut for the OS
            ...
            print(f"Error during real copy-paste: {e}")
            return False
